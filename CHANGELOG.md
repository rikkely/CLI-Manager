...
- **终端选区删除光标位置修复**：删除输入行中间的选中文字后，光标保留在删除位置，不再跳到剩余文本末尾。
...
- **系统资源面板历史保留修复**：侧栏在资源监控与其他面板之间来回切换时，不再清空 CPU/网络/GPU 的历史采样数据，切回后趋势图在原历史上继续追加。
...
### 终端
- **按项目聚焦终端切换空白修复**：按项目聚焦终端开启后，切换项目只隐藏不属于当前项目的终端 Tab，不再卸载对应 xterm 实例，避免终端文字空白、必须调整窗口尺寸才恢复。
